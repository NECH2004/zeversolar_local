...
from custom_components.zeversolar_local.const import DOMAIN, CONF_SERIAL_NO, OPT_DATA_INTERVAL
from custom_components.zeversolar_local.config_flow import ZeverSolarFlowHandler, ZeverSolarOptionsFlowHandler
...
async def test_ZeverSolarFlowHandler_async_step_user_user_input_id_is_none():
    """Tests the user step wtih data but inverter id cannot be loaded"""
    result_flow_handler = ZeverSolarFlowHandler()
...
async def test_ZeverSolarFlowHandler_async_step_user_user_input_id_loaded_new_inverter():
    ...
    with patch("custom_components.zeversolar_local.zeversolar_api.zs.inverter_id") as api_mock:
        ...
        with patch("custom_components.zeversolar_local.config_flow.ZeverSolarFlowHandler._async_current_entries") as current_entries_mock:
...
async def test_ZeverSolarFlowHandler_async_step_user_user_input_id_loaded_duplicate_inverter():
    ...
    data = {
        CONF_HOST: "TEST_HOST",
        CONF_SERIAL_NO: zever_inverter_id
    }
    ...
    with patch("custom_components.zeversolar_local.zeversolar_api.zs.inverter_id") as api_mock:
        ...
        with patch("custom_components.zeversolar_local.config_flow.ZeverSolarFlowHandler._async_current_entries") as method_mock:
...
async def test_ZeverSolarFlowHandler_get_id():
    """Tests the _get_id method returing an id"""
    result_flow_handler = ZeverSolarFlowHandler()
...
async def test_ZeverSolarFlowHandler_test_url():
    """Tests the _test_url method returing an id"""
    result_flow_handler = ZeverSolarFlowHandler()
